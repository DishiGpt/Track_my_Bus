...
import { 
  Users, Map as MapIcon, ShieldCheck, LogOut, 
  CheckCircle, LayoutDashboard, Settings as SettingsIcon, 
  Activity, Bus, Navigation, AlertCircle, Search, User, Loader2
} from 'lucide-react';
...
const AdminApp: React.FC = () => {
  ...
  const [currentPage, setCurrentPage] = useState<'dashboard' | 'live-map' | 'roster' | 'settings'>('dashboard');
  ...
  useEffect(() => {
    if (isLoggedIn) fetchDrivers();
  }, [isLoggedIn]);

  const fetchDrivers = async () => {
    const mockData = [
      { id: 'EMP101', name: 'Ramesh Kumar', routeNumber: localStorage.getItem('duty_EMP101'), status: 'Offline' },
      { id: 'EMP102', name: 'Suresh Singh', routeNumber: localStorage.getItem('duty_EMP102'), status: 'Offline' },
      { id: 'EMP103', name: 'Mukesh Sharma', routeNumber: localStorage.getItem('duty_EMP103'), status: 'Offline' },
      { id: 'EMP104', name: 'Dinesh Patel', routeNumber: localStorage.getItem('duty_EMP104'), status: 'Offline' },
      { id: 'EMP105', name: 'Vikram Singh', routeNumber: localStorage.getItem('duty_EMP105'), status: 'Offline' },
    ];
    setDrivers(mockData);
  };
  ...
  const handleLogin = (creds: any) => {
    setLoading(true);
    setTimeout(() => {
      if (creds.username === 'admin' && creds.password === 'udaipur2026') {
        setIsLoggedIn(true);
      } else {
  ...
  const assignDuty = () => {
    ...
    localStorage.setItem(`duty_${selectedDriver}`, selectedRoute);
    setToast(`Operator ${selectedDriver} deployed to Route ${selectedRoute}`);
    setDrivers(prev => prev.map(d => d.id === selectedDriver ? { ...d, routeNumber: selectedRoute, status: 'Assigned' } : d));
    ...
    <div className="h-full w-full bg-slate-100 flex font-sans overflow-hidden">
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="bg-white px-12 py-8 flex justify-between items-center shrink-0 border-b border-slate-100">
          <div>
            <h2 className="text-3xl font-black text-slate-900 tracking-tight uppercase">
              {currentPage === 'dashboard' ? 'Command Center' : currentPage.replace('-', ' ')}
            </h2>
        ...
        <div className="flex-1 overflow-y-auto p-12 space-y-12 custom-scrollbar">
          {currentPage === 'dashboard' && (
            <>
              <section className="grid grid-cols-4 gap-8">
                ...
                <StatCard icon={<Navigation className="text-indigo-600"/>} label="Grid Sectors" value="10" trend="All Green" />
                <StatCard icon={<Users className="text-green-600"/>} label="Operators" value="12" trend="Live Sync" />
                <StatCard icon={<AlertCircle className="text-slate-400"/>} label="Inhibitors" value="0" trend="No Threats" />
              </section>

              <div className="grid grid-cols-12 gap-10">
                <section className="col-span-8">
                  <RosterPage drivers={drivers} minimal onAssign={(id) => setSelectedDriver(id)} />
                </section>
                ...
                <section className="col-span-4 bg-indigo-600 rounded-[48px] p-10 text-white shadow-[0_30px_60px_rgba(79,70,229,0.2)]">
                  ...
                  <div className="space-y-6">
                    <div>
                      ...
                      <select value={selectedDriver || ''} onChange={e => setSelectedDriver(e.target.value)} className="w-full bg-white/10 border border-white/20 p-4 rounded-2xl outline-none font-bold text-white focus:bg-white/20 transition-all">
                    ...
                    <div>
                      ...
                      <select value={selectedRoute || ''} onChange={e => setSelectedRoute(e.target.value)} className="w-full bg-white/10 border border-white/20 p-4 rounded-2xl outline-none font-bold text-white focus:bg-white/20 transition-all">
                        <option value="" disabled className="text-slate-900">Select Sector...</option>
                        {UDAIPUR_ROUTES.map(r => <option key={r.routeNumber} value={r.routeNumber} className="text-slate-900">Route {r.routeNumber}</option>)}
                      </select>
                    </div>
                    <button 
          ...
          {currentPage === 'roster' && <RosterPage drivers={drivers} />}
